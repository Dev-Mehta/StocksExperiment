...
class TestFeed(bt.feeds.PandasData):
	lines = ('proximalLine', 'distalLine' ,)
	params = (('proximalLine', 12),('distalLine', 13))
...
class TestStrategy(bt.Strategy):
	...
	def __init__(self):
		...
		self.buyprice = None
		self.buycomm = None
		self.scores = self.datas[0].proximalLine
		self.trades = None
		self.size = 10
		self.stop_loss = 0.1
		self.trailing = True
	...
	def next(self):
		...
		if not self.position:
			...
			if self.scores[0]:
				...
				self.order = self.buy()
				self.sell(exectype=bt.Order.StopTrailLimit, price=self.datas[0].distalLine[0], trailpercent=0.05)
				# self.log("SL Placed " + str(self.datas[0].distalLine[0]))
...
datapath = os.path.join(modpath, 'infy_prep_weekly.csv')
